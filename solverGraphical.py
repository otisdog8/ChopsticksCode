...
class Position:
    ...
    def printPosition(self):
      return str(self.position[0:2]) + " " + str(self.position[2:4])

    def convertMoveToWords(self, position):
      result = None
      offset = 0 if self.turn else 2
      if position.position[offset] != self.position[offset]:
        # split
        result = f"split from {str(self.position[offset:offset+2])} to {str(position.position[offset:offset+2])}"
      else:
        for i in range(2):
          if position.position[2 - offset + i] != self.position[2 - offset + i]:
          # Find which one changed in the otehr
            for j in range(2):
              if position.position[2 - offset + i] - self.position[2 - offset + i] == position.position[offset + j] or self.position[2 - offset + i] - position.position[2 - offset + i] + 5 == position.position[offset + j] or position.position[2 - offset + i] - self.position[2 - offset + i] + 5 == position.position[offset + j]:
                hand1 = "right" if j else "left"
                hand2 = "right" if i else "left"
                result = f"{hand1} hand hit {hand2} hand and changed hands from {str(self.position[2 - offset:4 - offset])} to {str(position.position[2 - offset:4 - offset])}"
            # Find which one hit.
      if result == None:
        result = "Cannot generate string to describe move - previous thing describes state of hands AFTER you move"
      return result
...
while True:
    ...
    for ind, child in enumerate(currentpos.children):
        print(ind, " ", child.printPosition(), " ", currentpos.convertMoveToWords(child))
    while True:
    ...
    if bestwinpos is not None:
        ...
        for (child, weight) in weights:
            if total <= num and num <= total + weight:
                ...
                total += weight
    print("AI PLAYED ", newpos.convertMoveToWords(currentpos))
    print("CURRENT POSITION ", currentpos.printPosition())

...
class ChopstickComputer:
    def __init__(self) -> None:
        ...
        for i, r in enumerate(rref):
            self.maxscore = max(self.maxscore, r[-1])
            self.minscore = min(self.minscore, r[-1])
    ...
    def gameLoop(self):
        ...
        while True:
            ...
            while True:
                ...
                if move == "explore":
                    for i in cache[
                        input("Enter cache compatible position string: ")
                    ].children:
            ...
            if bestwinpos is not None:
                ...
                for child in newpos.children:
                    if child.won != True:
                        weights.append(
                            (child, 100 / (weightscore + child.blunderscore))
                        )
    ...
    def getBestMove(self, position):
        # Internal function to get best move from a position
        newpos = position
        cache = self.cache
        minscore = self.minscore
        maxscore = self.maxscore
    ...
    def move(self, position):
        # Moves based on an array of 4 then returns the same array of 4 without the turn
        postring = ""
        for s in position:
            postring += str(s)
        postring += "2"
        pos = self.cache[postring]
        newpos = self.getBestMove(pos)
        posarr = []
        for i in range(4):
            posarr.append(int(str(newpos)[i]))
        return posarr
